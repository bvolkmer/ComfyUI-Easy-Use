...
class imageInsetCrop:
    ...
    def crop(self, measurement, left, right, top, bottom, image=None):
        ...
        inset_left, inset_right, inset_top, inset_bottom = get_new_bounds(
        ...
        log_node_info(
            "Image Inset Crop",
            f"Cropping image {width}x{height} width inset by {inset_left},{inset_right}, "
...
class imageSize:
    ...
    def image_width_height(self, image):
        ...
        if width is not None and height is not None:
            ...
            result = (0, 0)
        return {
            "ui": {"text": "Width: " + str(width) + " , Height: " + str(height)},
            "result": result,
        }
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "mode": (
                    "BOOLEAN",
                    {"default": True, "label_on": "max", "label_off": "min"},
                ),
            }
        }
...
class imageScaleToNormPixels:
    ...
    def scale(self, image, upscale_method, scale_by):
        ...
        (image,) = upscale_image_cls().upscale(
            image, upscale_method, width, height, "disabled"
        )
        return (image,)
...
class imagePixelPerfect:
    ...
    def INPUT_TYPES(s):
        RESIZE_MODES = [
            ResizeMode.RESIZE.value,
            ResizeMode.INNER_FIT.value,
    ...
    def join(self, images, mode):
        ...
        if mode == "vertical":
            ...
            image = torch.transpose(
                torch.transpose(images, 1, 2).reshape(1, n * w, h, c), 1, 2
            )
        return (image,)
    ...
    def doit(self, images, row, column):
        ...
        for i in range(row):
            for j in range(column):
                new_images.append(
                    self.crop(
                        images, sub_width, sub_height, j * sub_width, i * sub_height
                    )
                )
...
class imageSplitTiles:
    ...
    def doit(
        self, image, overlap_ratio, overlap_offset, tiles_rows, tiles_cols, norm=True
    ):
        height, width = image.shape[1:3]
        ...
        for i in range(tiles_rows):
            for j in range(tiles_cols):
                ...
                (mask,) = feather_mask_cls().feather(
                    mask, fearing_left, fearing_top, fearing_right, fearing_bottom
                )
                masks.append(mask)
        ...
        return (
            tiles,
            masks,
            (overlap_w, overlap_h, tile_w, tile_h, tiles_rows, tiles_cols),
            total,
        )
...
class imageTilesFromBatch:
    ...
    def doit(self, tiles, masks, overlap, index):
        ...
        y = (
            tile_h * (index // tiles_cols) - overlap_h
            if tiles_rows > 1 and index > tiles_cols - 1
            else 0
        )
    ...
    def concat(self, image1, image2, direction, match_image_size):
        ...
        if match_image_size:
            image2 = torch.nn.functional.interpolate(
                image2, size=(image1.shape[2], image1.shape[3]), mode="bilinear"
            )
        if direction == "right":
...
class imageRemBg:
    ...
    def INPUT_TYPES(self):
        return {
            "required": {
                "images": ("IMAGE",),
                "rem_mode": (
                    ("RMBG-2.0", "RMBG-1.4", "Inspyrenet", "BEN2"),
                    {"default": "RMBG-1.4"},
                ),
                "image_output": (
    ...
    def remove(
        ...
        if rem_mode == "RMBG-2.0":
            ...
            from ..modules.briaai.rembg import (
                BriaRMBG,
                preprocess_image,
                postprocess_image,
            )
            ...
            if rem_mode in cache:
                ...
                model_path = get_local_filepath(
                    model_url, REMBG_DIR, rem_mode + "." + suffix
                )
                net = BriaRMBG()
        ...
        if add_background != "none":
            ...
            _canvas = Image.new(
                "RGB",
                _layer.size,
                (255, 255, 255) if add_background == "white" else (0, 0, 0),
            )
...
class imageColorMatch(PreviewImage):
    ...
    def color_match(
        ...
        if method in ["wavelet", "adain"]:
            result_images = (
                wavelet_color_fix(tensor2pil(image_target), tensor2pil(image_ref))
                if method == "wavelet"
                else adain_color_fix(tensor2pil(image_target), tensor2pil(image_ref))
            )
            ...
            for i in range(batch_size):
                image_target_np = (
                    images_target_np if batch_size == 1 else images_target[i].numpy()
                )
                image_ref_np_i = (
                    image_ref_np if image_ref.size(0) == 1 else images_ref[i].numpy()
                )
                try:
                    image_result = cm.transfer(
                        src=image_target_np, ref=image_ref_np_i, method=method
                    )
...
class imageDetailTransfer:
    ...
    def transfer(
        ...
        if target.shape[1:] != source.shape[1:]:
            target_tensor = comfy.utils.common_upscale(
                target_tensor, width, height, "bilinear", "disabled"
            )
        ...
        gaussian_blur = GaussianBlur(
            kernel_size=(kernel_size, kernel_size), sigma=(blur_sigma, blur_sigma)
        )
        ...
        if mode == "add":
            ...
            new_image = (
                1 - 2 * blurred_target
            ) * source_tensor**2 + 2 * blurred_target * source_tensor
...
class humanSegmentation:
    ...
    def parsing(
        self, image, confidence, method, crop_multi, prompt=None, my_unique_id=None
    ):
        mask_components = []
        if my_unique_id in prompt:
            if prompt[my_unique_id]["inputs"]["mask_components"]:
                mask_components = prompt[my_unique_id]["inputs"][
                    "mask_components"
                ].split(",")
        ...
        if method == "selfie_multiclass_256x256":
            ...
            if method in cache:
                ...
                model_path = get_local_filepath(
                    MEDIAPIPE_MODELS["selfie_multiclass_256x256"]["model_url"],
                    MEDIAPIPE_DIR,
                )
            ...
            with mp.tasks.vision.ImageSegmenter.create_from_options(
                options
            ) as segmenter:
                for img in image:
            ...
            if method in cache:
                ...
                model_path = get_local_filepath(
                    HUMANPARSING_MODELS["parsing_lip"]["model_url"], onnx_path
                )
            ...
            if method in cache:
                ...
                model_path = get_local_filepath(
                    HUMANPARSING_MODELS["human-parts"]["model_url"], human_parts_path
                )
        ...
        bbox = [[0, 0, 0, 0]]
        if crop_multi > 0.0:
            output_image, mask, bbox = imageCropFromMask().crop(
                output_image, mask, crop_multi, crop_multi, 1.0
            )

        return (output_image, mask, bbox)


class humanSegmentation2:
    SELECTIONS: Final[list[str]] = [
    ...
    selection_index: Final[dict[str, int]] = {s: i for i, s in enumerate(SELECTIONS)}
    ...
    def parsing(
        ...
        mask_components: list[int] = [self.selection_index[s] for s in selection.split(",")]
        if method == "selfie_multiclass_256x256":
            try:
                import mediapipe as mp
            except:
...
class imageCropFromMask:
    ...
    def cropimage(self, original_images, masks, crop_size_mult, bbox_smooth_alpha):
        ...
        self.max_bbox_width = self.smooth_bbox_size(
            self.max_bbox_width, curr_max_bbox_width, bbox_smooth_alpha
        )
        self.max_bbox_height = self.smooth_bbox_size(
            self.max_bbox_height, curr_max_bbox_height, bbox_smooth_alpha
        )
        ...
        for i, (mask, img) in enumerate(zip(masks, original_images)):
            ...
            if i > 0:
                center = self.smooth_center(
                    self.prev_center, curr_center, bbox_smooth_alpha
                )
            else:
                center = curr_center
            ...
            crop_transform = CenterCrop(
                (self.max_bbox_height, self.max_bbox_width)
            )  # swap the order here if necessary
    ...
    def crop(self, image, mask, image_crop_multi, mask_crop_multi, bbox_smooth_alpha):
        cropped_images, bounding_boxes = self.cropimage(
            image, mask, image_crop_multi, bbox_smooth_alpha
        )
        cropped_mask_image, _ = self.cropimage(
            self.mask2image(mask), mask, mask_crop_multi, bbox_smooth_alpha
        )
...
class imageUncropFromBBOX:
    ...
    def uncrop(
        ...
        if len(bbox) > len(original_image):
            print(
                f"Warning: Dropping excess bounding boxes. Expected {len(original_image)}, but got {len(bbox)}"
            )
        ...
        for i in range(len(original_image)):
            ...
            paste_region = self.bbox_to_region(
            ...
            crop = crop.resize(
                (
                    round(paste_region[2] - paste_region[0]),
                    round(paste_region[3] - paste_region[1]),
                )
            )
            ...
            if use_square_mask:
                ...
                mask_block = Image.new(
                    "L",
                    (
                        paste_region[2] - paste_region[0],
                        paste_region[3] - paste_region[1],
                    ),
                    255,
                )
                ...
                if optional_mask is None:
                    raise ValueError(
                        "optional_mask is required when use_square_mask is False"
                    )
...
class loadImageBase64:
    ...
    def load_image(
        self, base64_data, image_output, save_prefix, prompt=None, extra_pnginfo=None
    ):
        ...
        if image_output in ("Hide", "Hide/Save"):
            return {"ui": {}, "result": (new_images, mask)}

        return {"ui": {"images": results}, "result": (new_images, mask)}
...
class removeLocalImage:
    ...
    def remove(self, any, file_name):
        ...
        if self.hasFile:
            PromptServer.instance.send_sync(
                "easyuse-toast", {"content": "Removed SuccessFully", "type": "success"}
            )
        else:
            PromptServer.instance.send_sync(
                "easyuse-toast", {"content": "Removed Failed", "type": "error"}
            )
        return ()
...
class loadImagesForLoop:
    ...
    RETURN_TYPES = ByPassTypeTuple(
        tuple(["FLOW_CONTROL", "INT", "IMAGE", "MASK", "STRING", any_type, any_type])
    )
    RETURN_NAMES = ByPassTypeTuple(
        tuple(["flow", "index", "image", "mask", "name", "value1", "value2"])
    )
    ...
    def load_images(
        ...
        **kwargs,
        ...
        outputs = [
            kwargs.get("initial_value1", None),
            kwargs.get("initial_value2", None),
        ]
...
class makeImageForICRepaint:
    ...
    def make(
        self, image_1, direction, pixels=0, image_2=None, mask_1=None, mask_2=None
    ):
        if image_2 is None:
            image_2 = self.emptyImage(image_1.shape[2], image_1.shape[1])
            mask_2 = torch.full(
                (1, image_1.shape[1], image_1.shape[2]),
                1,
                dtype=torch.float32,
                device="cpu",
            )
        ...
        if img1_h != img2_h and img1_w != img2_w:
            ...
            image_1 = comfy.utils.common_upscale(
                image_1, width, height, "bicubic", "disabled"
            )
            image_1 = image_1.movedim(1, -1)
        ...
        image = (
            torch.cat((image_1, image_2), dim=2)
            if direction == "left-right"
            else torch.cat((image_1, image_2), dim=1)
        )
...
NODE_CLASS_MAPPINGS = {
    ...
    "easy humanSegmentation2": humanSegmentation2,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "easy humanSegmentation2": "Human Segmentation (templateable)",
